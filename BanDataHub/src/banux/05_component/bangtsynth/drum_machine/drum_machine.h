 ...
 *   - BG_FlashMgr.h (Flash读写)
 *   - FreeRTOS task.h (xTaskGetTickCount)
 *
 * 宏控制:
...
#define DM_MAX_PATTERNS         8       /* 最大Pattern存储槽数 */
...
/* Flash 偏移 (在 Storage 分区内) */
#define DM_FLASH_BASE           0x600000
#define DM_CONFIG_OFFSET        (DM_FLASH_BASE + 0x000000)
#define DM_PATTERN_OFFSET       (DM_FLASH_BASE + 0x001000)
#define DM_PATTERN_SLOT_SIZE    0x001000    /* 4KB per slot */
#define DM_SOUNDBANK_OFFSET     (DM_FLASH_BASE + 0x010000)
#define DM_SOUNDBANK_MAX_SIZE   (0x200000 - 0x010000)  /* ~1.9MB */
...
/**
 * @brief 保存Pattern到Flash槽位
 * @param slot  槽号 (0 ~ DM_MAX_PATTERNS-1)
 * @return 0=成功, -1=失败
 */
int DrumMachine_SavePattern(uint8_t slot);

/**
 * @brief 从Flash槽位加载Pattern
 * @param slot  槽号 (0 ~ DM_MAX_PATTERNS-1)
 * @return 0=成功, -1=失败或无数据
 */
...
/**
 * @brief 启动鼓机音源下载 (通过BLE数据包协议)
 * @param file_size  音源文件大小(字节)
 * @return 0=成功, -1=失败
 */
