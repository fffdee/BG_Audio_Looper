...
/* 接口实例 */
SoundBank_Manager soundbank_manager __attribute__((section(".data"))) = {
    .Init = soundbank_init,
...
static void soundbank_note_on(uint8_t note, uint8_t velocity, uint8_t program)
{
    ...
    } else if (g_current_format == SOUNDBANK_FORMAT_SAMPLER) {
...
static void soundbank_pitch_bend(uint8_t channel, int16_t value)
{
    if (g_current_format == SOUNDBANK_FORMAT_SF2) {
        sf2_pitch_bend(channel, value);
    }
}
...
{
    if (g_current_format == SOUNDBANK_FORMAT_SF2) {
        sf2_channel_cc(channel, cc_num, value);
    }
}
...
static uint8_t soundbank_read_active_samples(short *data, uint32_t count)
{
    ...
    switch (g_current_format) {
        ...
        case SOUNDBANK_FORMAT_SAMPLER:
            return Sampler_ReadSamples(data, count);
