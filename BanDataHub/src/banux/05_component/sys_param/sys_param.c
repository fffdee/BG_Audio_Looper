...
SysParam_Status_t SysParam_ApplyToAudio(void) {
    ...
    /* effect_graph removed - audio chain node settings not applied to runtime */
    PARAM_DBG("[PARAM] Audio chain apply skipped (effect_graph removed)\n");
...
static void SysParam_PrintEffectParams(GraphNode_t *node, uint8_t node_id) {
    (void)node_id;  /* effect_graph removed - only static storage available */
