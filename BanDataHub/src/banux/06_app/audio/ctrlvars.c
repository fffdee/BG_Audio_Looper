...
const AUDIO_EFF_PARAMAS EFFECT_TAB[10]={
...
/* 内存优化：只保留效果图实际使用的5个EQ单元 */
/* const 优化：聚合数组存储在 Flash 中而非 RAM，节省 20+12+20+8+64=124 字节 RAM */
EQUnit * const eq_unit_aggregate[5] 			=  {
	/* 索引0: USB/BT路径EQ (节点14) */
	&gCtrlVars.music_out_eq_unit,
	/* 索引1-4: ADC通道独立EQ (节点4-7) */
...
//	#endif
	...
	/* ========== 初始化5个实际使用的EQ单元 ========== */
	/* 新数组结构: [0]=music_out_eq, [1-4]=ADC独立EQ */

	/* 索引0: music_out_eq_unit (节点14: USB_BT_EQ) - 使用EQ_DEFAULT_TABLE[1]的配置 */
	eq_unit_aggregate[0]->ct			= NULL;
	eq_unit_aggregate[0]->enable		= EQ_DEFAULT_TABLE[0][1];  /* 从旧索引1获取配置 */
	...
	for(j = 0; j < eq_unit_aggregate[0]->filter_count; j++)
	{
		...
		/* 修正类型：如果是BAND_PASS(5)，改为PEAKING(0) */
	...
	/* 索引1-4: 4个独立ADC通道EQ单元 (节点4-7: Guitar L/R, Mic L/R) */
	/* 内存优化：使用3段EQ以降低内存占用 */
	for(i = 1; i < 5; i++)
	{
		eq_unit_aggregate[i]->ct			= NULL;
		eq_unit_aggregate[i]->enable		= 1;  /* 默认启用 */
		eq_unit_aggregate[i]->channel		= 1;  /* 单声道 */
		eq_unit_aggregate[i]->filter_count  = 3;  /* 3段EQ（节省内存） */
		eq_unit_aggregate[i]->pregain		= 0;  /* 无预增益 */
		eq_unit_aggregate[i]->filter_params = eq_param_aggregate[i];  /* 使用对应的参数缓冲区 */
		
		/* 初始化为平坦响应（0dB增益） */
		for(j = 0; j < 3; j++)
		{
			eq_unit_aggregate[i]->eq_params[j].enable	= 1;
			eq_unit_aggregate[i]->eq_params[j].f0		= 1000;  /* 默认中频 */
			eq_unit_aggregate[i]->eq_params[j].Q		= 100;   /* Q值1.0 */
			eq_unit_aggregate[i]->eq_params[j].gain		= 0;     /* 0dB */
			eq_unit_aggregate[i]->eq_params[j].type		= 0;     /* PEAKING类型 */
	...
	/* 额外确保：修正music_out_eq_unit所有频段类型 */
...
void GlobalMclkSet(void)
{
	...
	{
	    //��Clock_AudioPllClockSet��ʹ����PLL_CLK_USER_DEF���˴���Ҫ�������õ�ʱ��Դ��ѡ��PLL_CLOCK1������PLL_CLOCK2!!
...
void DefaultParamgsInit(void)
{
	...
	gCtrlVars.AutoRefresh         = 0;//����ʱ��Ч���������ı䣬��λ�����Զ���ȡ��Ч���ݣ�1=������λ����0=����Ҫ��λ����ȡ
...
void AudioAnaChannelSet(int8_t ana_input_ch)
{
	...
	gCtrlVars.pga0_line2_l_en = 0;
	gCtrlVars.pga0_line2_r_en = 0;
	#if (CFG_RES_MIC_SELECT == 0)//��Ϊlinein3Ӧ��
