...
extern const AUDIO_EFF_PARAMAS EFFECT_TAB[10];
...
//-----system function--------------------------//
/* const 优化：将聚合数组存储在 Flash 中而非 RAM，节省内存 */
extern EQUnit * const eq_unit_aggregate[5];  /* 内存优化：只保留效果图实际使用的5个EQ单元 */
extern DRCUnit * const drc_unit_aggregate[3];
extern EQFilterParams * const eq_param_aggregate[5];  /* 内存优化：只保留5个EQ的参数缓冲区 */
