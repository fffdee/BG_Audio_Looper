...
#if !HW_DRV_LCD_EN
/* GUI component removed - provide stub drawing primitives */
static inline void Gui_DrawPoint(uint16_t x, uint16_t y, uint16_t color) { (void)x; (void)y; (void)color; }
static inline void Gui_DrawLine(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color) { (void)x1; (void)y1; (void)x2; (void)y2; (void)color; }
#endif
