...
{
    // 电池图标位置（右上角）
    uint16_t bat_x = LCD_WIDTH - 90;  // 距离右侧40像素
    uint16_t bat_y = 1;               // 距离顶部2像素

    // 绘制电池外壳
    BG_lcd.Box(bat_x, bat_y, 23, 10, 0xFFFF);  // 电池主体（白色边框）
    BG_lcd.Box(bat_x + 24, bat_y + 2, 3, 6, 0xFFFF);  // 电池正极（白色）

    // 绘制电量格数（每格3像素宽，从左向右绘制）
    uint8_t i;
    // 假设最大电量格数为3（与原逻辑保持一致）
    for(i = 0; i < statusbar_data.battery_grid; i++)
    {
        // 从左向右排列：每格间隔1像素，起始位置为电池内左侧+2像素
        uint16_t fill_x = bat_x + 2 + (i * 4);  // 3像素宽 + 1像素间隔
        BG_lcd.Box(fill_x, bat_y + 2, 3, 6, 0x07E0);  // 绿色填充
    }

    // 绘制百分比（保留原注释逻辑，可根据需要实现）
    // char bat_str[4];
    // sprintf(bat_str, "%d%%", statusbar_data.battery_percent);  // 假设存在电量百分比数据
