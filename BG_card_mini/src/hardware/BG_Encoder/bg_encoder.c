...
BG_Encoder BG_encoder = {
	...
	.encoder = encoder_button_state,					//获取旋转的值
...
uint8_t encoder_button_state(void) {
    ...
    uint8_t direction_state = 0; // 0表示无变化，1表示顺时针，2表示逆时针

    // 如果A的状态改变了，那么检测B的状态以确定旋转方向
    if (task_run.A != task_run.last_A) {
        // 去抖动逻辑（示例，可能需要根据实际情况调整）
        // ...

        // 检测B相状态，确定旋转方向
        if (task_run.B != task_run.last_A) {
            // 顺时针旋转
            if (task_run.mode == OVER_MODE ||
                (task_run.mode == LIMIT_MODE && task_run.encoder_position < task_run.max_value)) {
                task_run.encoder_position++;
                direction_state = 1; // 设置方向状态为顺时针
                DBG("LEFT\n");
            }
        } else {
            // 逆时针旋转
            if (task_run.mode == OVER_MODE ||
                (task_run.mode == LIMIT_MODE && task_run.encoder_position > task_run.min_value)) {
                task_run.encoder_position--;
                direction_state = 2; // 设置方向状态为逆时针
                DBG("RIGHT\n");
            }
        }
