/**
 * flash_manager.c - Flash management implementation
 *
 * Manages multiple W25Q64 NOR Flash chips:
 * - Flash #0: 1MB system settings + 7MB Looper
 * - Flash #1: 8MB storage
 */
...
FlashStatus_t FlashManager_Init(void)
{
    ...
    /* Set all CS pins inactive */
    FLASH_NOR0_CS_DISABLE();
    FLASH_NOR1_CS_DISABLE();
    FLASH_WP_DISABLE();
    
    /* Create Flash #0 device instance */
    ...
    /* Create Flash #1 device instance */
    ...
    /* System Settings partition - Flash #0, 1MB */
...
FlashStatus_t FlashManager_Read(PartitionType_t type, uint32_t offset, uint8_t *buf, uint32_t len)
{
    ...
    /* Check offset and length against partition size */
...
FlashStatus_t FlashManager_EraseSector(PartitionType_t type, uint32_t offset)
{
    ...
    /* Check offset against partition size */
    if (offset >= part->size) {
        return FLASH_ERROR_PARAM;
    }
...
FlashStatus_t FlashManager_Test(uint8_t flash_id)
{
    ...
    if (flash_id == FLASH_DEV_0) {
        test_addr = PARTITION_LOOPER_START; /* Test Looper partition first, then System settings */
    } else {
        test_addr = 0;
    }
