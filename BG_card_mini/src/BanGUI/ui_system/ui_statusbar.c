...
static void UI_StatusBar_DrawBattery(void)
{
    ...
    uint16_t bat_x = LCD_WIDTH - 60;  // 40 pixels from right
    uint16_t bat_y = 1;               // 2 pixels from top

    // Draw battery outline
    BG_lcd.Box(bat_x, bat_y, 23, 10, 0xFFFF);  // Battery body (white border)
    BG_lcd.Box(bat_x + 24, bat_y + 2, 3, 6, 0xFFFF);  // Battery positive terminal (white)

    // Draw battery level grids (each grid 3px wide, draw from left to right)
    uint8_t i;
    // Assume max grid count is 3 (consistent with original logic)
    for(i = 0; i < statusbar_data.battery_grid; i++)
    {
        // Each grid separated by 1px, start at battery left +2px
        uint16_t fill_x = bat_x + 2 + (i * 4);  // 3px width + 1px gap
        BG_lcd.Box(fill_x, bat_y + 2, 3, 6, 0x07E0);  // Green fill
    }

    // Draw percentage (keep original comment logic, implement if needed)
    // char bat_str[4];
    // sprintf(bat_str, "%d%%", statusbar_data.battery_percent);  // Assume battery percentage data exists
    // BGUI_tool.ShowString(bat_x - 24, bat_y, (uint8_t*)bat_str, 0xFFFF);
}
...
static void draw_dac_icon(void)
{
    ...
    /* Headphone icon with blue color, or Speaker icon with white color */
...
void UI_StatusBar_Init(void)
{
    ...
    statusbar_data.dac_output = UI_DAC_SPKR;  /* Default to Speaker output*/
...
void UI_StatusBar_ScanDetect(void)
{
    ...
    if (GPIO_RegOneBitGet(UI_DET_HP_PORT, UI_DET_HP_PIN)) {
        ...
        new_dac |= UI_DAC_SPKR;  /* Default to Speaker output */
    }
    ...
    /* ADC 12-bit to 0-100 volume level */
