 ...
 * @brief    Shell IO manager implementation - automatically switch CDC/BLE interface and provide access protection
 *****************************************************************************
 */
...
/*******************************************************************************
 * External dependencies - System Tick acquisition
 *******************************************************************************/
/* Use FreeRTOS tick */
#include "FreeRTOS.h"
#include "task.h"
...
void ShellIOManager_Process(void)
{
    ...
    /* Check if each interface has data */
    ...
    switch (g_io_manager.state)
    {
        case SHELL_IO_STATE_IDLE:
            /* Idle state: detect which interface has data */
            if (cdc_has_data)
            {
        ...
        case SHELL_IO_STATE_ACTIVE:
            ...
            if (g_io_manager.active_io == SHELL_IO_CDC)
            {
                ...
                {
                    /* No data on both sides and timeout, return to idle */
                    g_io_manager.state = SHELL_IO_STATE_IDLE;
                }
            }
    ...
    /* Call Shell processing function */
    Shell_Process();
...
{
    /* If already locked by another interface, return failure */
    if (g_io_manager.state == SHELL_IO_STATE_LOCKED && 
...
{
    /* Lock state does not allow switching */
    if (g_io_manager.state == SHELL_IO_STATE_LOCKED)
    {
        return 0;
    }
...
void ShellIOManager_UpdateActivity(ShellIOType_t io_type)
{
    if (g_io_manager.active_io == io_type)
    {
        ...
        /* If in locked state, also update lock time */
        if (g_io_manager.state == SHELL_IO_STATE_LOCKED)
        {
            g_io_manager.lock_tick = GET_TICK_MS();
        }
