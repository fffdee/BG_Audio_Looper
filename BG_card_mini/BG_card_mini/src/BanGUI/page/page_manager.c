...
#include "bg_menu_slider.h"  // 寮曞叆鏂扮殑BG_MenuSlider鎺т欢
...
void list_page();
void list_page_in();
void bg_menu_slider_page();  // 鏂板鐨凚G_MenuSlider婕旂ず椤甸潰

BG_Page_Table table[MAX_PAGE] = {

    //Selectable UI icon.
//	{"bg menu slider", BG_MENU_SLIDER_PAGE, HOME_PAGE, HOME_PAGE, HOME_PAGE, BG_MENU_SLIDER_PAGE, SETUP, bg_menu_slider_page},
	{"home page", HOME_PAGE, HOME_PAGE, HOME_PAGE, HOME_PAGE, HOME_PAGE, SETUP, home_page},
...
static void menu_setup_callback(void) {
   BGUI_tool.ShowString(10, 100, "Setup Selected!", 0xFFFF00);
}

static void menu_bt_callback(void) {
    // 钃濈墮璁剧疆閫昏緫
    BGUI_tool.ShowString(10, 100, "BT Selected!", 0x07E0);
}

static void menu_audio_callback(void) {
    // 闊抽璁剧疆閫昏緫
    BGUI_tool.ShowString(10, 100, "Audio Selected!", 0xF800);
}

static void menu_system_callback(void) {
    // 绯荤粺璁剧疆閫昏緫
    BGUI_tool.ShowString(10, 100, "System Selected!", 0x001F);
}
...
void home_page() {
    static uint8_t need_redraw = 0;
    static uint8_t ui_drawn = 0; // 鏍囪UI鏄惁宸茬粯鍒�
...
//        need_redraw = 1;
    ...
    // 鍙湪绗竴娆＄粯鍒跺浐瀹歎I鍏冪礌
    if (!ui_drawn) {
        BGUI_tool.ShowString((LCD_WIDTH - 9 * 8) / 2, 10, "MAIN MENU", 0xFFFF);
        BGUI_tool.ShowString(5, LCD_HEIGHT - 15, "L/R:Move Enter:OK", 0x7BEF);
        ui_drawn = 1;
    }
//
//    // 澶勭悊鎸夐敭杈撳叆(宸﹂敭/鍙抽敭/纭)
...
void list_page()
{
    ...
    if (BG_page.Data.table[BG_page.Data.running_id].setup == 1)
    {
       //  printf("BG_page.Data.running_id is %d\n", BG_page.Data.running_id);
        ...
        BGUI_tool.ShowString(20, 60, "Press Enter->Detail", 0xFFFF00);
        BGUI_tool.ShowString(20, 80, "Press Exit->Home", 0xFFFF00);
...
void bg_menu_slider_page() {
    ...
    if (BG_page.Data.exit_pressed) {  // 退出键 - 返回主页
        BG_page.SetPage(&BG_page, HOME_PAGE);
        BG_page.Data.exit_pressed = 0;
        need_init = 1;  // 下次进入时重新初始化
        return;
    }
