...
public class EqControlActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (paramCache.isSyncComplete() && paramCache.has(BleProtocol.CMD_EQ)) {
            applyCachedEqParams();
        } else {
            queryEqParams();
        }
    ...
    private void applyCachedEqParams() {
        BleParamCache cache = BleParamCache.getInstance();
        byte[] eqRaw = cache.getEqRaw();
        if (eqRaw == null || eqRaw.length < 3) return;

        int bandCount = eqRaw[0] & 0xFF;
        int pregain = (eqRaw[1] & 0xFF) | ((eqRaw[2] & 0xFF) << 8);

        if (currentNode < nodesBands.size()) {
            List<EqCurveView.EqBand> bands = nodesBands.get(currentNode);
            bands.clear();
            for (int i = 0; i < bandCount && i < 10; i++) {
                int base = 3 + i * 10;
                if (base + 10 > eqRaw.length) break;
                EqCurveView.EqBand band = new EqCurveView.EqBand();
                band.gain = (short)((eqRaw[base] & 0xFF) | ((eqRaw[base + 1] & 0xFF) << 8));
                band.f0 = (eqRaw[base + 2] & 0xFF) | ((eqRaw[base + 3] & 0xFF) << 8) |
                          ((eqRaw[base + 4] & 0xFF) << 16) | ((eqRaw[base + 5] & 0xFF) << 24);
                band.Q = (short)((eqRaw[base + 6] & 0xFF) | ((eqRaw[base + 7] & 0xFF) << 8));
                band.type = eqRaw[base + 8] & 0xFF;
                band.enable = eqRaw[base + 9] != 0;
                bands.add(band);
            }
            loadCurrentNodeBands();
            if (eqCurveView != null) eqCurveView.invalidate();
        }
        android.util.Log.d("EqControl", "Applied cached EQ params (no BLE query needed)");
    }
