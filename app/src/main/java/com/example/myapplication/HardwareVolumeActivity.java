...
public class HardwareVolumeActivity extends BaseActivity {
    ...
    /**
     * 从连接时同步缓存中直接读取音量参数（无需重新查询设备）
     */
    private void applyCachedVolumeParams() {
        int[] raw = BleParamCache.getInstance().getVolumeParams();
        if (raw == null) {
            // 缓存数据不完整，回退到主动查询
            queryVolumeParams();
            return;
        }
        // 设备发送顺序: [mic1, mic2, guitar1, guitar2, output]
        // UI 顺序:      [guitar1, guitar2, mic1, mic2, output]
        final int[] uiOrder = {raw[2], raw[3], raw[0], raw[1], raw[4]};
        runOnUiThread(() -> {
            for (int i = 0; i < uiOrder.length && i < volumeSeekBars.length; i++) {
                if (volumeSeekBars[i] != null) volumeSeekBars[i].setProgress(uiOrder[i]);
                if (volumeValueTexts[i] != null) volumeValueTexts[i].setText(String.valueOf(uiOrder[i]));
            }
        });
    }
