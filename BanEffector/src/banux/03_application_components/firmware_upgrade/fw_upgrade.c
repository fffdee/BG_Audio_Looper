...
/* banux2 component catalog descriptor (metadata-only). The upgrade flow is
 * driven explicitly via the FwUpgrade_* facade, so no init/process hook is
 * wired here; registration only exposes it to `banux -i` component listing. */
BANUX_COMPONENT_DEFINE(g_banux_component_firmware_upgrade,
                       "firmware_upgrade", "1.0.0",
                       BANUX_COMPONENT_APPLICATION, 1,
                       "Firmware upgrade: dual-partition boot + CDC");
