...
//static uint8_t g_reverb_static_mem[REVERB_SIZE] __attribute__((aligned(4)));
//static ReverbContext *g_pReverbStaticCtx = NULL;
//
//void AudioEffectReverbInit(ReverbUnit *unit, uint8_t channel, uint32_t sample_rate)
//{
//#if CFG_AUDIO_EFFECT_MIC_REVERB_EN
...
//    }
//
//    // �����޸ģ����ٵ���pvPortMallocFromEnd��ֱ���þ�̬�ڴ�
//	if(unit->ct == NULL)
//	{
//        // ��ʼ����̬������ָ�루����һ�ε���ʱ��ֵ��
...
void AudioEffectPlateReverbInit(PlateReverbUnit *unit, uint8_t channel, uint32_t sample_rate)
{
	...
	if(unit->ct == NULL)
	{
		...
		DBG("PlateReverbContext mem malloc ok! %ld\n",PLATE_REVERB_SIZE);
		if(unit->ct == NULL)
		{
			unit->enable = 0;
...
uint32_t AudioEffectReverbApply2(ReverbUnit *unit, int16_t *pcm_in, int16_t *pcm_out, uint32_t n)
{
	if((unit->enable) && (unit->ct != NULL))
	{
		return reverb_apply(unit->ct, pcm_in, pcm_out, n);
	}
}
/*
****************************************************************
...
uint32_t AudioEffectPlateReverbApply2(PlateReverbUnit *unit, int16_t *pcm_in, int16_t *pcm_out, uint32_t n)
{
	if((unit->enable) && (unit->ct != NULL))
	{
		return plate_reverb_apply(unit->ct, pcm_in, pcm_out, n);
	}
}
/*
****************************************************************
	...
	if((unit->enable) && (unit->ct != NULL))
	{
		...
		if(&gCtrlVars.music_out_eq_unit == unit)
		{
			if(mainAppCt.EqModeBak != mainAppCt.EqMode)
			{
				...
				du_efft_fadeout_sw(pcm_out, n, channel);
				//eq_clear_delay_buffer(unit->ct);//��ǰ��EQģʽ������filter��Ŀ�������Ҫ�����д���
