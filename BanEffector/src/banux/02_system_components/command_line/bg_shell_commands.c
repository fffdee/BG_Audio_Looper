 ...
 * @author   BanEffector Porting
 * @brief    Shell command module implementation (MVS BP10 / nds32)
 *
 * 本文件由 banux2 二代框架移植到 BanEffector 音频效果器工程：
 *   1. sys     模块：改用 SDK reset.h（Reset_McuSystem 软复位）+ 平台信息
 *   2. VFS     模块：整块 #if VFS_EN 守卫（BanEffector 无文件系统, VFS_EN=0）
 *   3. boot    模块：改用 FwUpgrade_RebootToBootloader()（写烧录魔数后复位）
 *   4. 删除 STM32 HAL / fs_sd / app_bl 等 3D 打印机平台残留依赖
 *****************************************************************************
 */

#include <stdint.h>     /* uint16_t/uint32_t for SDK reset.h */
#include <string.h>
#include <stdlib.h>
#include <stdio.h>
#include "reset.h"        /* SDK: Reset_McuSystem() 软复位 */
#include "bg_shell.h"
#include "drv_init.h"
#include "vfs.h"        /* 虚拟文件系统API */
#include "drv_fs.h"     /* 驱动文件系统API */
#include "drv_device.h" /* 驱动设备管理 */
#include "fw_upgrade.h" /* FwUpgrade_RebootToBootloader() */
...
static int sys_info(int argc, char *argv[])
{
    ...
    Shell_Print("\r\nSystem Information:\r\n");
    Shell_Print("  Device:    BanEffector (BANBOX_1_0_V2)\r\n");
    Shell_Print("  SoC:       MVS BP10 series (nds32)\r\n");
    Shell_Print("  Storage:   NAND Flash + PSRAM\r\n");
    Shell_Printf("  Shell IO:  %s\r\n", Shell_GetIOName());
    Shell_Printf("  Banux:     %s\r\n\r\n", BANUX_VERSION_STRING);
    return 0;
}
...
static int sys_mem(int argc, char *argv[])
{
    (void)argc; (void)argv;
    Shell_Print("\r\nMemory Status:\r\n");
    Shell_Print("  Code Flash: on-chip (App after bootloader)\r\n");
    Shell_Print("  NAND Flash: W25N02 (storage)\r\n");
    Shell_Print("  PSRAM:      ESP-PSRAM64H (audio buffers)\r\n\r\n");
    return 0;
}
 ...
 *   boot  通过 FwUpgrade_RebootToBootloader() 写烧录魔数后复位，进入 Bootloader
 *===========================================================================*/
static int boot_enter(int argc, char *argv[])
{
    ...
    FwUpgrade_RebootToBootloader();   /* 写烧录魔数 + Reset_McuSystem(), 不返回 */
