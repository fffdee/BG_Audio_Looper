...
#define MEMSET(s, c, n) __builtin_memset ((s), (c), (n))
	...
	/* Bootloader handoff: if magic present at 0x20000000, the bootloader
	 * already copied .data and cleared .bss before jumping here.
	 * We must skip the copy to avoid the NDS32 IBus/SBus mutual-exclusion
	 * deadlock (executing from flash while reading .data from flash). */
	{
		volatile uint32_t *handoff = (volatile uint32_t *)0x20000000UL;
		if (*handoff == 0xDEADBEEFUL) {
			*handoff = 0;  /* clear magic so cold boot still works */
			return;
		}
	}
...
------------------------------------------------------------*/
	...
	/* BanBox: Chip_MemInit() is intentionally NOT called here.
	 * MPU (incl. 0x80000000 -> 0x20048000 BB_EM mapping) is configured by
	 * Chip_Init(1) in main(). Calling Chip_MemInit() before Chip_Init(1)
	 * leaves a stale MPU state that later causes an Imprecise Bus Error
	 * when the BT stack does memset(0x80000000). Matches BT_Audio_APP. */
