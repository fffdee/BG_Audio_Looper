 ...
 void A2dp_Decode(void)
{
	...
	if( RT_SUCCESS == audio_decoder_can_continue() )
	{
		...
		if(audio_decoder_decode() == RT_SUCCESS)
		{
			...
			//AudioDAC1DataSet(audio_decoder->song_info->pcm_addr, audio_decoder->song_info->pcm_data_length  * audio_decoder->song_info->num_channels);
			uint16_t OutPut_bufx[audio_decoder->song_info->pcm_data_length];
			uint8_t i;
			for(i=0;i< audio_decoder->song_info->pcm_data_length ;i++)
				OutPut_bufx[i] = (uint16_t)audio_decoder->song_info->pcm_addr[i];
			AudioDAC1DataSet(OutPut_bufx, audio_decoder->song_info->pcm_data_length);
		}else
