...
/**
 * @brief Update audio spectrum with new audio data
 * @param audio_left Left channel audio samples
 * @param audio_right Right channel audio samples
 * @param sample_count Number of samples
 * @note Should be called from background task, not audio interrupt!
 */
void UI_System_UpdateSpectrum(const int16_t* audio_left,
                               const int16_t* audio_right,
                               uint16_t sample_count);

/**
 * @brief Update spectrum from BG_AudioManager output buffer
 * @note Call this periodically in main loop (e.g., every 50ms)
 */
void UI_System_UpdateSpectrumFromAudioManager(void);
