...
#define EFFECT_GRAPH_BUFFER_SIZE    256     /* 处理缓冲区大小(samples) */
 ...
 ******************************************************************************/
/* 源节点: 产生数据 */
typedef uint16_t (*NodeSourceFunc_t)(EffectNode_t *node, int32_t *out_buf, uint16_t max_len);

/* 处理节点: 处理数据 (支持多输入) */
typedef void (*NodeProcessFunc_t)(EffectNode_t *node, 
                                   int32_t **in_bufs, uint8_t in_count,
                                   int32_t *out_buf, uint16_t len);

/* 输出节点: 消费数据 */
typedef void (*NodeSinkFunc_t)(EffectNode_t *node, int32_t *in_buf, uint16_t len);
...
struct EffectGraph {
    ...
    /* 共享处理缓冲区 */
    int32_t             shared_buffer[EFFECT_GRAPH_MAX_NODES][EFFECT_GRAPH_BUFFER_SIZE];
};
