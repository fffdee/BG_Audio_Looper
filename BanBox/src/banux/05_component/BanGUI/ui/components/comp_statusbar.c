...
void UI_StatusBar_Draw(void)
{
    ...
    /* Draw BT icon if applicable */
    if (statusbar_data.bt_status != UI_BT_OFF) {
        BGUI_tool.ShowString(5, 1, (statusbar_data.bt_status == UI_BT_CONNECTED) ? "BT" : "bt", 0x07E0);
    }
    
    /* Draw ADC/DAC icons */
    uint16_t icon_x = 25;
    if (statusbar_data.adc_source & UI_ADC_MIC) {
        BGUI_tool.ShowString(icon_x, 1, "MIC", 0x07E0);
        icon_x += 25;
    }
    if (statusbar_data.adc_source & UI_ADC_GUITAR) {
        BGUI_tool.ShowString(icon_x, 1, "GTR", 0x07E0);
        icon_x += 25;
    }
    if (statusbar_data.dac_output & UI_DAC_HP) {
        BGUI_tool.ShowString(icon_x, 1, "HP", 0x07E0);
    }
...
{
    /* Draw default BanBox logo text */
    BGUI_tool.ShowString(40, UI_STATUSBAR_HEIGHT + 15, "BanBox", UI_COLOR_WHITE);
}
