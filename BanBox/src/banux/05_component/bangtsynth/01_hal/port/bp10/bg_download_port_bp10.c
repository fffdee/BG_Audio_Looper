 ...
 * @brief BG_Download_Port �?BP10 平台实现 (数据包协�?
 * 
 * 功能:
 * - 通过 Shell IO (USB CDC / BLE SPP) 接收音源数据�?
 * - 解析 DL 协议数据�?(同步�?+ 命令 + 序列�?+ 数据 + CRC)
 * - 发�?ACK/NAK 响应
 * - 提供 bg_download_port_read() 接口�?soundbank_download() 调用
 * 
 * 协议流程:
 * 1. Shell 命令 "sb -d <size>" 启动下载会话
 * 2. 设备发�?READY 响应
 * 3. 主机发�?DL_CMD_DATA 数据�?(每包最�?4096 字节)
 * 4. 设备校验 CRC �?返回 ACK/NAK
 * 5. soundbank_download() 将有效载荷写�?Flash
 * 6. 主机发�?DL_CMD_END 结束传输
 * 
...
static int dl_recv_bytes(uint8_t *buf, uint16_t need, uint32_t timeout_ms)
{
    ...
    while (received < need && elapsed < timeout_ms) {
        ...
        if (n > 0) {
            ...
            bg_task_delay(DL_POLL_INTERVAL_MS);
