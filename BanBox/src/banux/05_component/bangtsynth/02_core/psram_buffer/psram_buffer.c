...
{
    PSRAM_BufferInfo_t *buffer;
    FlashDevice_t *nand_dev, *psram_dev;
    uint8_t *temp_buffer;
    uint32_t remaining = size;
    uint32_t chunk_size;
    uint32_t current_nand_offset;
    ...
    /* 分块传输数据 */
    current_nand_offset = nand_offset;
    ...
    while (remaining > 0) {
        ...
        /* �?NAND 读取数据到临时缓冲区 */
        ret = (BG_ERR)nand_dev->ops->read(nand_dev, current_nand_offset, temp_buffer, chunk_size);
        if (ret != SUCCESS) {
            BG_LOG_E(BG_LOG_TAG_PSRAM, "Failed to read from NAND: offset=0x%08X, size=%u",
                     current_nand_offset, chunk_size);
            free(temp_buffer);
            return ret;
        }
    ...
    BG_LOG_I(BG_LOG_TAG_PSRAM, "Data chunk loaded: buffer=%u, nand_offset=0x%08X, psram_offset=0x%08X, size=%u",
