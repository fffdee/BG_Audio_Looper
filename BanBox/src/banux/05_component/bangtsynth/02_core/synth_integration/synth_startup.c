...
static bool startup_step_nand_init(void) {
    ...
    /* NAND 初始化已在 SYNTH_SDNANDPSRAM_Init 中处理 */
    g_startup_state.nand_init_done = true;
    STARTUP_LOG_I("NAND store initialized");
    return true;
}
...
static bool startup_step_sf2_copy(void) {
    ...
    /* SF2 拷贝已在 SYNTH_SDNANDPSRAM_Init 中处理 */
    g_startup_state.sf2_copy_done = true;
    STARTUP_LOG_I("SF2 copy completed");
    return true;
}
...
static bool startup_step_storage_driver(void) {
    ...
    STARTUP_LOG_I("NAND storage driver installed");
    return true;
}
