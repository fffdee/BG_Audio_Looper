...
void SYNTH_SDNANDPSRAM_DeInit(void)
{
    ...
    BG_LOG_I(BG_LOG_TAG_SYNTH, "SD+NAND+PSRAM synthesizer integration deinitialized");
}
...
void SYNTH_SDNANDPSRAM_NoteOn(uint8_t note, uint8_t velocity, uint8_t program)
{
    ...
    /* 缓存未命中，需要从 NAND 加载数据 */
    ret = synth_locate_note_data(note, program, &nand_offset, &data_size);
    if (ret != SUCCESS) {
    ...
    /* 异步加载到 PSRAM */
    ret = PSRAM_LoadNoteData(alloc_result.buffer_id, nand_offset, data_size);
    if (ret != SUCCESS) {
