...
void showimage(uint8_t x,uint8_t y,uint8_t width,uint8_t high,const uint8_t *p){ //显示40*40 QQ图像
  	int row, col;  // 改为行、列命名，更清晰
	unsigned char picH,picL;
	//Lcd_Clear(BLACK); //清屏

	// 修正后的显示区域设置
	Lcd_SetRegion(x+2, y, (x+2)+width-1, y+high-1);		//坐标设置

	// 双层循环：先行后列，贴合ST7735 GRAM填充逻辑
	for(row=0; row<high; row++)  // 遍历每一行
	{
	    for(col=0; col<width; col++)  // 遍历当前行的每一列
	    {
	        // 按行优先读取数据，与显示顺序完全匹配
	        picL=*(p + row*width*2 + col*2);	//数据低位在前
